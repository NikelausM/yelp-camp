...
class CampgroundController {
	...
	static async campgroundIndexGet(req, res) {
		...
		try {
			...
			const count = await Campground.countDocuments().exec();
	...
	static setCampgroundAttr(target, source, attributes) {
	...
	static async campgroundUpdatePut(req, res) {
		try {
			...
			CampgroundController.setCampgroundAttr(campground, req.body.campground, ["name", "price", "image", "description", "location", "lat", "lng"]);
